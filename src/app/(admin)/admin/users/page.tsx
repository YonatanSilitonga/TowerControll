...
const columns: Column<UserAdmin>[] = [
  ...
  { header: "Username", render: (r) => <span className="font-mono text-xs font-bold text-slate-800 dark:text-white">{r.username}</span> },
  ...
  { header: "Status", className: "w-24", render: (r) => (
    <span className={`inline-flex rounded-full px-2 py-0.5 text-[10px] font-bold ${
      r.is_active ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400"
...
export default function AdminUsersPage() {
  return (
    <AdminCrudPage
      ...
      idKey="id_user"
      listFn={adminUser.list}
      createFn={adminUser.create}
      updateFn={async (id, data) => {
        // Update role only — password handled separately
        const { password, ...rest } = data;
        return adminUser.updateRole(id, rest.role);
      }}
