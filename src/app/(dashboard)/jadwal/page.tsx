...
export default function JadwalPage() {
  ...
  useEffect(() => {
    if (jenisFilter === "outgoing" && !["all", "1", "2", "3"].includes(ritaseFilter)) {
      setRitaseFilter("all");
    }
  }, [jenisFilter]);
  ...
  useEffect(() => {
    ...
    setNewRitase((cur) => {
      ...
      const needUpdate =
        cur.id_drop_point === 1 ||
        cur.stops.some(
          (s) => (s.jenis_stop === "gudang" && s.id_gudang === 1) || (s.jenis_stop === "gateway" && s.id_drop_point === 1)
        );
      ...
      return {
        ...
        stops: cur.stops.map((s) => {
          if (s.jenis_stop === "gudang" && firstGudang)
            return { ...s, id_gudang: firstGudang.id_gudang, nama_lokasi: firstGudang.nama_gudang };
          if (s.jenis_stop === "gateway" && firstDP)
            return { ...s, id_drop_point: firstDP.id_drop_point, nama_lokasi: firstDP.nama_drop_point };
          return s;
        }),
      };
    });
  ...
  const handleGenerate = () => {
    ...
    generateMutation.mutate(payload, {
      onSuccess: (data) => {
        ...
        const count = (data as { total_generated?: number })?.total_generated ?? editableRoutes.length;
        showToast("success", `Berhasil menimpa & meng-generate ${count} ritase harian!`);
        refetch();
      },
      onError: (e) => {
  ...
  const handleSaveCreate = (e: React.FormEvent) => {
    ...
    const selectedDriver = activeDrivers.find((d) => d.id_driver === newRitase.id_driver);
    if (!selectedDriver) {
      showToast("error", "Driver yang dipilih sedang tidak aktif. Pilih driver lain yang aktif.");
      return;
    }
    const selectedVehicle = activeVehicles.find((k) => k.id_kendaraan === newRitase.id_kendaraan);
    if (!selectedVehicle) {
      showToast("error", "Kendaraan yang dipilih sedang tidak tersedia. Pilih kendaraan lain.");
      return;
    }
  ...
  const outgoingCount = ritases?.filter((r) => r.jenis_ritase === "outgoing").length ?? 0;
  const incomingCount = ritases?.filter((r) => r.jenis_ritase === "incoming").length ?? 0;
                            ...
                            <div className="flex flex-1 min-w-0 items-center gap-1.5">
                              ...
                              {((stop.jumlah_koli ?? 0) > 0 || (stop.jumlah_ecer ?? 0) > 0 || (stop.jumlah_high_value ?? 0) > 0) && (
                                <span className="shrink-0 text-[10px] text-amber-600 dark:text-amber-400">
                                  ...
                                  {(stop.jumlah_ecer ?? 0) > 0 && <span>·{stop.jumlah_ecer}E</span>}
                          ...
                          <div key={rIdx} className="flex flex-col justify-between rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-[#0f2847] hover:border-[#0c1e3a] dark:hover:border-blue-500/50 hover:shadow-md transition-all">
                            <div>
                              ...
                              <div className="mb-4 border-b border-slate-100 pb-3.5 dark:border-slate-700/50">
                                ...
                                {/* Date Badge */}
                                {route.tanggal_label && (
                                  <div className={`mb-2 inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-[10px] font-bold ${route.tanggal_label === "Hari Ini"
                                ...
                                {(route.jam_mulai || route.jam_selesai) && (
                                  <div className="flex items-center gap-2 rounded-lg bg-slate-50 dark:bg-slate-800/50 px-2.5 py-1.5">
                                    ...
                                    <span className="text-[11px] font-semibold text-slate-600 dark:text-slate-300">
                                      {route.jam_mulai?.slice(0, 5) ?? "?"} – {route.jam_selesai?.slice(0, 5) ?? "?"}
                                    </span>
                                  </div>
                                )}
                              </div>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm">
              <div className="w-full max-w-xl max-h-[90vh] flex flex-col rounded-lg bg-white p-5 dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                ...
                <form onSubmit={handleSaveCreate} className="flex-1 overflow-y-auto mt-4 space-y-4 pr-1">
                  ...
                  {(() => {
                    const selectedDriver = activeDrivers.find((d) => d.id_driver === newRitase.id_driver);
                    const jenis = masterOptions?.driver_jenis?.find(
                      (dj) => dj.id_driver === newRitase.id_driver && dj.ritase_ke === newRitase.ritase_ke
                    )?.jenis ?? "outgoing";
                    const jamConfig = masterOptions?.jam_ritase?.find(
                      (j) => j.jenis === jenis && j.ritase_ke === newRitase.ritase_ke
                    );
                    if (jamConfig) {
                      return (
                ...
                <form onSubmit={handleSaveEdit} className="flex-1 overflow-y-auto mt-4 space-y-5 pr-1">
                  {editingRitase.status === "berjalan" && (
                    <div className="flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 dark:border-amber-800 dark:bg-amber-950/40">
                      <AlertTriangle className="h-4 w-4 shrink-0 text-amber-500" />
                      <p className="text-xs font-medium text-amber-700 dark:text-amber-300">
                        Ritase sedang berjalan. Hanya perhentian (stops) yang bisa diubah.
                      </p>
                    </div>
                  )}
                  <div className="grid grid-cols-2 gap-4">
            ...
            >
              ...
              <div>
                <p className="text-sm font-bold text-white">
                  {toast.type === "success" ? "Perubahan Berhasil Disimpan! 🎉" : "Gagal Menyimpan"}
                </p>
