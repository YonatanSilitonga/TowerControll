        ...
        <DetailModal onClose={() => setSelected(null)}>
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <h3 className="mt-0.5 text-base font-bold text-slate-900">{selected.label}</h3>
            </div>
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 rounded-lg bg-slate-50 p-3 text-xs">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Indikator</p>
              <p className="mt-0.5 font-medium capitalize text-slate-700">{selected.indikator}</p>
            </div>
            <div>
              <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Nilai</p>
              <p className="mt-0.5 font-bold tabular-nums text-slate-800">{formatNumber(selected.nilai)}</p>
            </div>
          </div>
    ...
    <Card className="rounded-lg border-slate-200">
                ...
                >
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="mt-0.5 block text-[10px] opacity-70">
                      {KATEGORI_LABEL[al.kategori] ?? al.kategori} · {relTime(al.waktu)}
                    </span>
                  </span>
      ...
      {selected && (
        <DetailModal onClose={() => setSelected(null)}>
          ...
          <p className="mt-1 text-xs text-slate-400">Terjadi {relTime(selected.waktu)}</p>
